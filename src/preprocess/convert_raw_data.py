...
def convert_data_to_json(base_dir, save_data=False, save_dict=False):
    stack_examples = [] # 转换后的全体数据
    pseudo_examples = []    # 转换后的半监督数据
    test_examples = []  # 转换后的初赛测试集

    stack_dir = os.path.join(base_dir, 'train') # 存放未处理的train数据的目录
    pseudo_dir = os.path.join(base_dir, 'pseudo')   # 存放未处理的pseudo数据的目录
    test_dir = os.path.join(base_dir, 'test')   # 存放未处理的test数据的目录


    # 处理所有的训练样本
    for i in trange(201):  # 1000个训练样本

        # 读取第i个训练样本的text文件，得到第i个训练样本的text
        with open(os.path.join(stack_dir, f'{i}.txt'), encoding='utf-8') as f:
            text = f.read()

        # 读取第i个训练样本的ann文件，得到第i个训练样本的全部label
        labels = []
        ...
        # 将第i个样本的text和labels等信息放到字典中，再加到转换后的全体数据中
        # 这里'pseudo': 0表示不是半监督数据
    ...
    for i in trange(201, 301):    # 500个半监督数据

...
ENTITY_TYPES = ['Chemical', 'Container']
...
def convert_crf_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
                        max_seq_len, ent2id):
    # InputExample：`set_type=train/dev/...`、`text`、`labels`、`pseudo=0/1`、`distant_labels`
    ...
    if set_type == 'train':
        ...
        for ent in entities:
            ...
            if ent_start == ent_end:
                label_ids[ent_start] = ent2id['S-' + ent_type]  # single
            else:
                label_ids[ent_start] = ent2id['B-' + ent_type]  # begin
                label_ids[ent_end] = ent2id['E-' + ent_type]    # end
                for i in range(ent_start + 1, ent_end):
                    label_ids[i] = ent2id['I-' + ent_type]  # inner
    ...
    # encode仅返回inputs_ids，encode_plus返回一下三条内容
    # input_ids：单词在词典中的编码
    # token_type_ids：区分两个句子的编码（上句子全为0，下句全为1）
...
def convert_span_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    if set_type == 'train':
        ...
        for _ent in entities:
            ...
            ent_start = _ent[-1]    # 开始下标
            ent_end = ent_start + len(_ent[1]) - 1  # 结束下标
